classdef Nosecone < handle
    ...
    methods
        function obj = Nosecone(input1,input2, input3, input4, input5, input6, input7, input8, input9, input10, input11, input12, input13)
            ...
            obj.Moment_of_Inertia_nc(obj.MASS, obj.L, obj.AF_ID, obj.t);
        end
        ...
        %Moment of Inertia 
        function Moment_of_Inertia_nc(obj, MASS, L, AF_ID, AF_t)
            AF_OD = AF_ID + 2 * AF_t;
            c = 1; %constant for assuming the moment for the nosecone is equivalent to that of a cylinder

            MoI_z = (3/10) * MASS * ((AF_OD / 2) ^ 2 - (AF_ID / 2) ^ 2);
            MoI_x = c * 1/12 * MASS * ((3*((AF_OD / 2)^2+(AF_ID / 2)^2)) + L^2); 
            MoI_y = c * 1/12 * MASS * ((3*((AF_OD / 2)^2+(AF_ID / 2)^2)) + L^2); 
